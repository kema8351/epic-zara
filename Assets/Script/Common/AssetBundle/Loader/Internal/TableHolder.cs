...
namespace Zara.Common.ExAssetBundle.Internal
{
    public class TableHolder : IAssetEntry
    {
        ...
        {
            coroutineOwner.Run(LoadAsync<T>(assetEntryKey.ToLower(), onLoaded));
        }
        ...
        IEnumerator LoadAsync<T>(string assetEntryKey, Action<T> onLoaded) where T : UnityEngine.Object
        {
            ...
            linker.LoadAsset<T>(assetEntryRecord, this.Table.GetNecessaryAssetBundleRecords(assetEntryKey), onLoaded);
        }

        public void LoadMultipleAssets<T>(string assetEntryKey, Action<T[]> onLoaded) where T : UnityEngine.Object
        {
            coroutineOwner.Run(LoadMultipleAssetsAsync<T>(assetEntryKey.ToLower(), onLoaded));
        }
        ...
        {
            coroutineOwner.Run(LoadPrefabAsync<T>(assetEntryKey.ToLower(), onLoaded));
        }
        ...
        IEnumerator LoadPrefabAsync<T>(string assetEntryKey, Action<T> onLoaded) where T : Component
        {
            ...
            linker.LoadPrefab<T>(assetEntryRecord, this.Table.GetNecessaryAssetBundleRecords(assetEntryKey), onLoaded);
        }
